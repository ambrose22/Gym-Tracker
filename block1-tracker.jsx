...
export default function App() {
  ...
  const persist = useCallback((updater) => {
    setState((prev) => {
      ...
      saveTimer.current = setTimeout(async () => {
        try {
          await window.storage.set(STORAGE_KEY, JSON.stringify(stateRef.current));
          setSaveStatus("Saved");
  ...
  const sendChat = async (text) => {
    ...
    try {
      ...
      persist((s) => ({ ...s, chat: [...s.chat, { role: "assistant", content: `Couldn't reach the coach (${e.message}). Try again.` }] }));
    } finally {
      setThinking(false);
    }
  };
  ...
  return (
    <div style={{ background: C.bg, minHeight: "100vh" }}>
      ...
      <button onClick={() => setChatOpen(true)} style={{
        ...
        background: C.amber, color: "#151A21", fontSize: 15, fontWeight: 900,
        boxShadow: "0 6px 20px rgba(0,0,0,0.45)", cursor: "pointer",
      }}>
        Ask coach
      </button>
